...
#define EndStopConnector ConnectorIO5
...
bool MoveToTorque(int8_t myTorqueSetting);
...
int32_t retractConstant = 12800;
int32_t RLVelocityConstant = 7000;
int32_t InjectionVelocityConstant = 7000;
int8_t InjectionTorqueTarget = 80;
...
void ReadInputButtons(){
	...
	if(!InputConnectorProx.InputFallen() && InjectConnector.InputRisen()){
		ConnectorLed.State(true);
		Delay_ms(10);
		ConnectorLed.State(false);
		Delay_ms(10);
...
void InjectCake(){
	...
	MoveToTorque(InjectionTorqueTarget);
...
void DisplayAnalog(){
	...
	if(serialPortOkay){
		...
		if(serialPortOkay) SerialPort.Send((bias * 100) - 100);
		if(serialPortOkay) SerialPort.Send("%");
		if(serialPortOkay) SerialPort.Send(" Torque Target = ");
		if(serialPortOkay) SerialPort.Send(bias * InjectionTorqueTarget);
		if(serialPortOkay) SerialPort.Send(",");
...
bool MoveToTorque(int8_t myTorqueSetting) {
	...
	if(serialPortOkay) SerialPort.Send("...Moving to torque: ");
	if(serialPortOkay) SerialPort.SendLine(myTorqueSetting);
	
	peaktorque = 0;

	// Move the motor down till we reach torque target value
	while(torque < myTorqueSetting){
		if(serialPortOkay) SerialPort.Send("Moving to ");
		if(serialPortOkay) SerialPort.SendLine(myTorqueSetting * bias);
		if(!atVelocity) atVelocity = MoveAtVelocity(InjectionVelocityConstant);
		GetMotorTorque();
		if(serialPortOkay) SerialPort.Send("current torque = ");
		if(serialPortOkay) SerialPort.SendLine(torque);
		peaktorque = torque;
		continue;
	}
	// Stop the motor after we have reached the torque target value
	MoveAtVelocity(0);
	if(serialPortOkay) SerialPort.SendLine("MoveDistance...Move Done");
	return true;
}
