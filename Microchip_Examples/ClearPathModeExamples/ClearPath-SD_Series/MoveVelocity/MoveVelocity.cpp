...
#define RaiseinputPin ConnectorDI8
#define LowerinputPin ConnectorA9
...
int main() {
	...
	RaiseinputPin.Mode(Connector::INPUT_DIGITAL);
	LowerinputPin.Mode(Connector::INPUT_DIGITAL);

    while (true) {
        // Move at 1,000 steps/sec for 2000ms
        while(RaiseinputPin.State()) {
			SerialPort.SendLine("Raise Detected...");
			MoveAtVelocity(3000);
		}
		MoveAtVelocity(0);
		while(LowerinputPin.State()) {
			SerialPort.SendLine("Lower Detected...");
			MoveAtVelocity(-3000);
		}
		MoveAtVelocity(0);
    }
